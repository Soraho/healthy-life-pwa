...
function ensureBaselineData() {
  let changed = false;
  const oldArchivedCount = state.archivedSeasons?.length || 0;
  state.archivedSeasons = (state.archivedSeasons || []).filter((season) => {
    return season.key !== "2026-06-19" && !String(season.title || "").includes("6/19-7/18");
  });
  if (state.archivedSeasons.length !== oldArchivedCount) changed = true;
...
function getSeasonBounds(date = getCurrentDate()) {
...
function getPreviousSeasonBounds(date = getCurrentDate()) {
      ...
      ${season.rows
        .map(
          (row, rowIndex) => {
            ...
            return `
            <li>
              <span class="history-medal" ${isEditable ? `contenteditable="true" data-edit-row-field="medal" data-edit-row-index="${rowIndex}"` : ""}>${escapeHtml(medal || "•")}</span>
              <div>
                <strong ${isEditable ? `contenteditable="true" data-edit-row-field="title" data-edit-row-index="${rowIndex}"` : ""}>${escapeHtml(title)}</strong>
                <p ${isEditable ? `contenteditable="true" data-edit-row-field="name" data-edit-row-index="${rowIndex}"` : ""}>${escapeHtml(name)}</p>
              </div>
              <span class="history-count" ${isEditable ? `contenteditable="true" data-edit-row-field="count" data-edit-row-index="${rowIndex}"` : ""}>${escapeHtml(count)}</span>
            </li>
          `;
          }
        )
...
function showUpdateNoticeIfNeeded() {
  ...
  if (sessionStorage.getItem(RELOAD_ON_UPDATE_KEY)) {
    sessionStorage.removeItem(RELOAD_ON_UPDATE_KEY);
  }
...
els.historyCard.addEventListener("blur", (event) => {
  ...
  const rowField = event.target.dataset.editRowField;
  const rowIndex = event.target.dataset.editRowIndex;
  if (titleIndex === undefined && rowField === undefined) return;
  ...
  if (rowField !== undefined) {
    const row = archivedSeason.rows[Number(rowIndex)];
    if (row) row[rowField] = value;
  }
...
ensureBaselineData();
localStorage.removeItem("healthy-life-test-date");
render({ fromRemote: true });
